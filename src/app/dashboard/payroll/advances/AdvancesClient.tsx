...
import { useBusinessStore } from '@/store/business-store'; // Added import
...
export default function AdvancesClient() {
    const { user, businessConfig } = useUserStore(); // Removed activeContextId from here
    const { primaryColor } = useThemeStore();
    const { selectedRange, activePreset } = useFilterStore(); // Kept original filter store destructuring
    const { activeContextId, activeContextName } = useBusinessStore(); // Added useBusinessStore and destructuring
    ...
    const contextName = isGlobal ? (user?.business_name || 'Global Business') :
                       (activeContextName || 'Outlet Context'); // Changed to use activeContextName
    ...
    const endStr = selectedRange.to.toISOString().split('T')[0];
    ...
    const handleIssueAdvance = async (e: React.FormEvent) => {
        e.preventDefault();
        if (isGlobal) {
            setGuardModal({ isOpen: true, action: 'Issue Advance Payment' });
            return;
        }
        ...
        createAdvanceMutation.mutate({
            ...
            business_id: user?.business_id || employee.business_id || '',
            outlet_id: isGlobal ? (employee.outlet_id || '') : activeContextId,
            outlet_name: isGlobal ? (employee.outlet_name || 'Global') : activeContextName,
            user_id: user?.id || '',
            status: 'pending'
        });
    ...
    const handleUpdateStatus = async () => {
        ...
        if (isGlobal) {
            setGuardModal({ isOpen: true, action: 'Authorize Disbursement' });
            return;
        }
    ...
    const handleDelete = async () => {
        if (isGlobal) {
            setGuardModal({ isOpen: true, action: 'Void Advance Record' });
            return;
        }
